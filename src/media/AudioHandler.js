...
var AudioHandler = function() {
	...
	var BEAT_MIN = 0.3; //level less than this is no beat
	...
	function update(){
		...
		for(var i = 0; i < binCount; i++) {
			waveData[i] = ((timeByteData[i] - 128) /128 )* 1.3; //ControlsHandler.audioParams.volSens;
		}
		...
		for(var i = 0; i < levelsCount; i++) {
			...
			levelsData[i] = sum / levelBins/256 * 1.3; //ControlsHandler.audioParams.volSens; //freqData maxs at 256
		...
		//BEAT DETECTION
		if (volume  > beatCutOff && volume > 0.5) { //ControlsHandler.audioParams.beatThreshold){
			onBeat();
			beatCutOff = volume *1.1;
			beatTime = 0;
		}else{
			if (beatTime <= 120) { //ControlsHandler.audioParams.beatHoldTime){
				beatTime ++;
			}else{
				beatCutOff *= 0.97; //ControlsHandler.audioParams.beatDecayRate;
				beatCutOff = Math.max(beatCutOff,0.5);
			}
		}
